from Services.DatabaseServices.RDBService import RDBDataTable
...
class ResourceBase():
    ...
    def update_by_template(self, template, transfer_json, context=None):
        result = self._data_table.update(template, transfer_json)
        if result:
            result = self._get_key(template)
    ...
    def delete(self, transfer_json):
        result = self._data_table.delete(transfer_json)
        return result


    def get_primary_key_columns(self):
        result = self._data_table.get_primary_key_columns()
        return result